...
const AdminPage = () => {
    ...
    const initialcoinsArg = {
        coinsArray: [],

    }
   const setcoinsData = (state, action) => {
        switch (action) {
            case "setcoinsArray":
                console.log(action.payload)
                return{
                    ...state,
                    coinsArray: [ ...action.item ]
                }
    ...
    const [ coinsData, coinsDataDispatch ] = useReducer( setcoinsData, initialcoinsArg );

    useEffect( () => {
     axios.get('https://coinscore-cc-backend.onrender.com/api/collections/coins/allcoin')
      .then(data => {coinsDataDispatch({type: "setcoinsArray", item: data.data.data}); setcoinsArray(data.data.data)})
        ...
        <div>
            ...
            { 
                ...
                deactivatedArr.map( (coin) => 
                   { return <ActivateCoinSlots key={coin._id} coin={coin} deletecoin={deletecoin} activatecoin={reviewcoin} ></ActivateCoinSlots>} )
            }
            <hr /> 
        </div>
        <div>
